...
def gunluk_trade_analiz(symbol: str, interval: str = "15m", hesap_buyuklugu: float | None = None,
    ...
    return {
        ...
        "Feature Contributions": {
            "EMA yönü": round(10 if ema21 > ema50 else -10, 1),
            "Momentum": round(8 if momentum else -8, 1),
            "MACD histogram yönü": round(8 if macd_up else -8, 1),
            "ADX yön teyidi": round(6 if adx14 >= 20 and ema21 > ema50 else -6, 1),
            "Hacim + VWAP teyidi": round(8 if rvol_ok and above_vwap else -8, 1),
            "Risk/Getiri": round(min(8, max(-8, (rr - min_risk_getiri) * 4)), 1),
        },
