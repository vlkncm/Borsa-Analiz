...
def tabloya_cevir(results):
    ...
    for item in results:
        tablo.append({
            ...
            "Açılış Fiyatı": item.get("acilis_fiyati", item.get("price", 0)),
            "Gün İçi Hedef": item.get("gun_ici_hedef", item.get("price", 0)),
            "Gün İçi Yükseliş %": item.get("gun_ici_yukselis_yuzde", 0),
            "Günlük Trade Teyit": item.get("gunluk_trade_teyit", "VERİ YETERSİZ"),
            "Günlük Trade Skoru": item.get("gunluk_trade_skoru", 0),
            "Mum Formasyonu": item.get("mum_formasyonu", "YOK"),
            "Mum Formasyon Yönü": item.get("mum_formasyon_yonu", "NÖTR"),
            "Mum Formasyon Teyidi": item.get("mum_formasyon_teyit", False),
            "AlphaTrend Yönü": item.get("alpha_trend_yonu", ""),
            "EMA20 Durumu": item.get("ema20_durumu", ""),
            "BBW %": item.get("bbw_yuzde", None),
            "BBW Durumu": item.get("bbw_durumu", ""),
            "MACD-V": item.get("macd_v", None),
            "MACD-V Sinyal": item.get("macd_v_sinyal", None),
            "MACD-V Durumu": item.get("macd_v_durumu", ""),
            ...
            "Hedef Olasılığı %": item.get("probability_target_before_stop"),
            "Olasılık Ufku — İşlem Günü": item.get("probability_horizon_days"),
            "Başarılılarda Medyan Süre": item.get("median_target_time_success_days"),
            "OOS Örnek Sayısı": item.get("probability_sample_size", 0),
            "Güven Aralığı": ("Yetersiz örnek" if item.get("probability_ci_low") is None else
                               f"%{item.get('probability_ci_low'):.0f} – %{item.get('probability_ci_high'):.0f}"),
            "Ufuk Olasılıkları": item.get("probability_by_horizon", {}),
            "Likidite Seviyesi": item.get("likidite_seviyesi", ""),
            "Ortalama Günlük İşlem Tutarı": item.get("ortalama_gunluk_islem_tutari", 0),
            "Hedef Alt Bant": item.get("hedef_alt_bant", 0),
            "Hedef Üst Bant": item.get("hedef_ust_bant", 0),
            "Canlı Uyarılar": item.get("canli_uyarilar", ""),
            "Olağandışı Hareket Riski": item.get("olaganustu_risk_puani", 0),
            "Resmi KAP Desteği": item.get("resmi_kap_destegi", ""),
            "Kovalama Engeli": item.get("kovalama_engeli", False),
            "Stres Çıkış Fiyatı": item.get("stres_cikis_fiyati", 0),
            "Fiyat Tipi": item.get("fiyat_tipi", ""),
            "Veri Kalite Notu": item.get("veri_kalite_notu", ""),
            "Uluslararası Faktör Puanı": item.get("uluslararasi_faktor_puani", 0),
            "Faktör Notu": item.get("faktor_notu", ""),
            ...
            "RSI-ST Dip Durumu": item.get("rsi_st_durum", "VERİ YOK"),
            "RSI-ST Yeni Sinyal": item.get("rsi_st_yeni_sinyal", False),
            "RSI-ST Trend Yukarı": item.get("rsi_st_trend_yukari", False),
            "RSI-ST RSI": item.get("rsi_st_rsi"),
            "RSI-ST RSI SMA": item.get("rsi_st_rsi_sma"),
            "RSI-ST Son Sinyal (Bar)": item.get("rsi_st_son_sinyal_bar"),
            "RSI-ST Zaman Dilimi": item.get("rsi_st_zaman_dilimi", "1G"),
            "RSI-ST Uyarı": item.get("rsi_st_not", ""),
...
def sade_ana_rapor(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "Hedef Olasılığı %": work.get("Hedef Olasılığı %", "Yetersiz örnek"),
        "Olasılık Ufku — İşlem Günü": work.get("Olasılık Ufku — İşlem Günü", "—"),
        "Başarılılarda Medyan Süre": work.get("Başarılılarda Medyan Süre", "—"),
        "OOS Örnek Sayısı": work.get("OOS Örnek Sayısı", 0),
        "Güven Aralığı": work.get("Güven Aralığı", "Yetersiz örnek"),
...
def main():
    ...
    print("Borsa Analiz Pro MAX v10.4.3 başladı:", datetime.now().strftime("%d.%m.%Y %H:%M"))
