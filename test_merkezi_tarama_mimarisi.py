...
from app_qt import MainWindow, ertesi_gun_tavan_gorunumu
...
class MerkeziTaramaMimarisiTests(unittest.TestCase):
    ...
    def test_next_day_ceiling_view_keeps_outside_bist30(self):
        source = pd.DataFrame({
            "Hisse": ["MEGMT"], "Aday Grubu": ["Teyit Bekleyen Aday"],
            "Önceki Kapanış": [24.0], "Tavan Aday Puanı": [62],
            "Ertesi Gün Tavan Olasılığı": ["Olasılık için yeterli geçmiş örnek bulunmuyor."],
        })
        result = ertesi_gun_tavan_gorunumu(source)
        self.assertEqual(result.iloc[0]["Hisse"], "MEGMT")
        self.assertIn("Tavan Aday Puanı", result.columns)
        self.assertIn("Ertesi Gün Tavan Olasılığı", result.columns)
