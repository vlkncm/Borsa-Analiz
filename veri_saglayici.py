...
def _normalize(df: pd.DataFrame | None) -> pd.DataFrame:
    ...
    if isinstance(out.index, pd.DatetimeIndex):
        # .values UTC'ye çevirip saat dilimini siler; yeniden İstanbul'a
        # yerelleştirme intraday barları üç saat geriye kaydırıyordu.
...
def download(symbol: str, period: str = "1mo", interval: str = "1d", **kwargs) -> pd.DataFrame:
    ...
    key = json.dumps(["v2", symbol, period, interval, adjusted], ensure_ascii=False)
