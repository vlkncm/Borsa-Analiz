...
class _Fake:
    def __init__(self, fail=False, source="Fintables"): self.fail = fail; self.source = source
    def get_daily_ohlcv(self, symbol, period="6mo"):
        ...
        return frame, VeriMetadatasi(self.source, pd.Timestamp.now().to_pydatetime(), frame.index[-1].to_pydatetime(), symbol)
...
class FintablesProviderTests(unittest.TestCase):
    ...
    def test_fallback_metadata(self):
        adapter = PrimaryFallbackAdapter(_Fake(True), _Fake(False, "Yahoo"))
        _, meta = adapter.get_daily_ohlcv("THYAO")
        self.assertTrue(meta.fallback_used)
        self.assertEqual("Yahoo", meta.source)

    def test_primary_metadata(self):
        adapter = PrimaryFallbackAdapter(_Fake(False), _Fake(False))
