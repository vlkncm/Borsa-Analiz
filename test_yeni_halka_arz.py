...
class EvrenYenilemeTests(unittest.TestCase):
    def test_yeni_sembol_gunluk_yenilemede_eklenir(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            html = " ".join('<a href="company/'+symbol.removesuffix('.IS')+'">' for symbol in base+["TKNKA.IS"])
            with patch("requests.get", return_value=SimpleNamespace(text=html, raise_for_status=lambda: None)):
            ...
            self.assertIn("TKNKA.IS", payload["symbols"])
